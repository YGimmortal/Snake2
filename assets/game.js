...
function requestFullscreen() {
  ...
  if (req) {
    try { req.call(el); } catch (e) { /* user gesture may be required; ignore */ }
  }
}
...
/* Pointer / touch: drag to change direction, short tap to pause */
...
function onPointerDown(e) {
  ...
  pointerDown = { x: e.clientX || (e.touches && e.touches[0].clientX), y: e.clientY || (e.touches && e.touches[0].clientY) };
  pointerMoved = false;
}
function onPointerMove(e) {
  ...
  if (x == null) return;
  const dx = x - pointerDown.x, dy = y - pointerDown.y;
...
function onPointerUp(e) {
  ...
  if (!pointerMoved && G.running && !G.over) {
    // short tap anywhere (except overlay buttons) → pause / unpause
...
function step() {
  ...
  // magnet: pull food one step closer if adjacent
  if (G.magnetUntil > now0 && G.food) {
    const dx = G.food.x - nx, dy = G.food.y - ny;
    if (Math.abs(dx) + Math.abs(dy) === 2) {
      if (Math.abs(dx) === 1) G.food.x -= Math.sign(dx);
      if (Math.abs(dy) === 1) G.food.y -= Math.sign(dy);
    }
  }
...
function render(dt) {
  ...
  if (G.food) {
    ...
    drawFoodDot(G.food, fd.color || getVar("--gold"), false);
  }
