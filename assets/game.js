...
/* Multi Food power: keeps G.extraFoods filled up to the equipped level's count, spawning/removing
   dots as the power is leveled up or down mid-run. */
function syncExtraFoods() {
  const wanted = activePowerCategoryEffects().multiFoodExtra;
  if (!G.extraFoods) G.extraFoods = [];
  if (G.extraFoods.length > wanted) G.extraFoods.length = wanted;
  while (G.extraFoods.length < wanted) {
    const exclude = [...G.snake, ...G.obstacles, ...(G.food ? [G.food] : []), ...(G.bonus ? [G.bonus] : []), ...G.extraFoods];
    G.extraFoods.push(randCell(exclude));
  }
}
...
function startGame() {
  ...
  // Cancel any previous render loop. Without this, PLAY AGAIN could start
  // multiple requestAnimationFrame loops and make the snake run too fast.
...
function updateHud() {
  ...
  if (chip) {
    ...
    if (pe.speedMult !== 1) {
      const pct = Math.round((pe.speedMult - 1) * 100);
      parts.push(pct > 0 ? `🐌+${pct}%` : `💨${pct}%`);
    }
    if (pe.multiFoodExtra > 0) parts.push(`🍇+${pe.multiFoodExtra} food`);
    if (pe.selfPassChance > 0) parts.push(`👻${Math.round(pe.selfPassChance * 100)}%`);
    if (pe.comboDecay > 0) parts.push(`🎯${Math.round(pe.comboDecay * 100)}%`);
    if (pe.magnetRangeBoost > 0 || pe.magnetDurationMs > 0) parts.push(`🧲+${pe.magnetRangeBoost.toFixed(1)}`);
    if (pe.shieldDurationMs > 0) parts.push(`🛡+${(pe.shieldDurationMs / 1000).toFixed(1)}s`);
...
function maybeSpawnBonus() {
  ...
  if (G.foodStreak > 0 && G.foodStreak % 5 === 0) {
    ...
    const cell = randCell([...G.snake, G.food, ...(G.extraFoods || []), ...G.obstacles]);
...
function endGame() {
  ...
  const runTime = Math.max(0, Math.floor((performance.now() - G.startedAt) / 1000));
  save.bestRunLength = Math.max(save.bestRunLength || 3, G.maxLength, G.snake.length);
  save.bestRunTime = Math.max(save.bestRunTime || 0, runTime);
  save.history.unshift({ score: G.score, coins: coinsFromScore, date: Date.now(), foods: G.foodsEaten, length: G.maxLength, time: runTime });
  ...
  const goRunTime = Math.max(0, Math.floor((performance.now() - G.startedAt) / 1000));
  ...
  if (goStats) goStats.textContent = `⏱ ${Math.floor(goRunTime / 60)}:${String(goRunTime % 60).padStart(2, "0")}   •   🍎 ${G.foodsEaten}   •   🐍 ${G.maxLength}   •   LV ${G.level}`;
...
function step() {
  ...
  if (hitsSelf || hitsObstacle) {
    const phantomChance = activePowerCategoryEffects().selfPassChance;
    const phantomSave = hitsSelf && !hitsObstacle && phantomChance > 0 && Math.random() < phantomChance;
    if (phantomSave) {
      toast("👻 PHANTOM", "phased through your own tail", "--purple");
    } else if (G.shieldUntil > performance.now()) {
      breakShield();
  ...
  if (G.magnetUntil > now0) {
    const magnetRange = 3 + Math.round(pe.magnetRangeBoost);
    const pullOne = (cell) => {
      if (!cell) return;
      const dx = cell.x - nx, dy = cell.y - ny;
      const dist = Math.abs(dx) + Math.abs(dy);
      if (dist >= 1 && dist <= magnetRange) {
        if (Math.abs(dx) >= Math.abs(dy) && dx !== 0) cell.x -= Math.sign(dx);
        else if (dy !== 0) cell.y -= Math.sign(dy);
        cell.x = Math.max(0, Math.min(COLS - 1, cell.x));
        cell.y = Math.max(0, Math.min(ROWS - 1, cell.y));
      }
    };
    pullOne(G.food);
    (G.extraFoods || []).forEach(pullOne);
  }

  syncExtraFoods();
  const eatenExtraIdx = (G.extraFoods || []).findIndex(f => nx === f.x && ny === f.y);
  const ateMainFood = G.food && nx === G.food.x && ny === G.food.y;
  const ateExtraFood = eatenExtraIdx !== -1;
  if (ateMainFood || ateExtraFood) {
    const now = performance.now();
    if (G.comboExpireAt > now) {
      G.combo = Math.min(G.combo + 1, 20);
    } else {
      // combo window had expired: Combo Guard keeps a fraction of the streak instead of a hard reset
      G.combo = Math.max(1, Math.round(G.combo * pe.comboDecay));
    }
    ...
    G.foodStreak++;
    G.foodsEaten++;
    G.maxLength = Math.max(G.maxLength, G.snake.length + 1);
    G.everAte = true;

    // One-time run milestones: reward good runs without flooding the player.
    while (G.score >= G.nextMilestone) {
      const milestone = G.nextMilestone;
      if (!G.milestoneClaimed.includes(milestone)) {
        const reward = Math.max(10, Math.round(milestone / 25));
        addCoins(reward);
        G.milestoneClaimed.push(milestone);
        toast("MILESTONE " + milestone, "+" + reward + "¢ bonus", "--gold");
        sfx.coin();
      }
      G.nextMilestone = milestone < 1000 ? milestone + 500 : milestone < 5000 ? milestone + 1000 : milestone + 2500;
    }
    grew = true;
    sfx.eat();
    ...
    if (ateMainFood) {
      G.food = randCell([...G.snake, ...(G.bonus ? [G.bonus] : []), ...G.obstacles, ...G.extraFoods]);
    } else {
      const exclude = [...G.snake, ...(G.bonus ? [G.bonus] : []), ...G.obstacles, ...(G.food ? [G.food] : []), ...G.extraFoods.filter((_, i) => i !== eatenExtraIdx)];
      G.extraFoods[eatenExtraIdx] = randCell(exclude);
    }
    maybeSpawnBonus();
    checkLevelUp();
  }
...
function render(dt) {
  ...
  if (G.food) {
    ...
    if (G.extraFoods && G.extraFoods.length) {
      G.extraFoods.forEach(f => drawFoodDot(f, fd.color || getVar("--gold"), false, fd.emoji || "🍎"));
    }
  }
...
function loop(ts, session = gameSession) {
  if (!G || G.over || session !== gameSession) return;
  if (!G.lastTick) G.lastTick = ts;
  const dt = Math.min(ts - G.lastTick, 100);
  G.lastTick = ts;
  if (!G.paused) {
    G.acc += dt;
  ...
  render(dt);
  animationFrameId = requestAnimationFrame((nextTs) => loop(nextTs, session));
}
